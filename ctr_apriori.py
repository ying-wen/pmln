...
opts.featindex = BASE_PATH + 'featindex.txt'
opts.model_name = 'criteo_distcrimination_concat'

### Paramerters for tuning
opts.gate_type = 'mul' # sum, mul
opts.activation_type = 'linear' # sigmoid, tanh, relu, linear etc.
opts.dropout = 0.1
...
x = e
l.append(x)
for i in range(opts.interaction_times):
    x = PairGateInteraction(
        gate_type=opts.gate_type, 
        activation=opts.activation_type, 
        dropout=opts.dropout, 
        name='interaction layer %d' % i
    )([x, e])
    if opts.highway:
        x = TimeDistributed(Highway())(x)
    x = KMaxTensorPooling(opts.sequence_length, name='kmaxpooling layer %d' % i)(x)
x = Lambda(lambda t: tf.reshape(t, [-1, opts.sequence_length * opts.embedding_size]))(x)
e = Lambda(lambda t: tf.reshape(t, [-1, opts.sequence_length * opts.embedding_size]))(e)
